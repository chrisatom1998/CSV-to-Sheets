...
| `transforms.js` | **Pure, DOM-free** data transforms shared by the popup, the parse worker, and the Node tests: `parseCSV(text, delim)` (RFC-4180-ish), `detectDelimiter(text)` (multi-row sampling sniffer), `decodeBytes(buffer)` (BOM/heuristic UTF-8 / UTF-16 decoding), `cleanNumeric(value, style)` (normalize currency/thousands, US or EU style; leave percents/text untouched), `detectNumberStyle(values)` (per-column `'us'`/`'eu'` sniffer), `splitRows(rawRows, {skip, firstRowHeader})`, `splitTargets(value)`, `toCSV(matrix)`, `sortRows(rows, keys)` (stable multi-key), `rowPassesFilter(row, filter)`, `consolidateRows(rows, groupIndex, {agg})` (merge matching rows; sum/avg/count/min/max), plus header-matching helpers `autoMatchIndex(target, csvHeaders)` (returns a column **index**, -1 if none) and `headerMatchConfidence(target, header)` (`'exact'`/`'similar'`/`'none'`). Exposed as `globalThis.Transforms` and `module.exports`; loaded via `<script>` before `popup.js` and via `importScripts` in `parse-worker.js`. |
| `parse-worker.js` | Web Worker that runs delimiter detection + CSV parsing off the main thread for large files (≥ 512 KB); `popup.js` (`parseAsync`) falls back to an inline parse when workers are unavailable or fail. |
